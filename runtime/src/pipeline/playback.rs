...
use crate::ipc::{
    ErrorCode, MediaTab, MediaType, MpdOutputInfo, MpdOutputsResponse, PlayerCmd,
...
pub async fn run_player_cmd(
    ...
    match cmd {
        ...
        PlayerCmd::Pause => {
            if via_mpd {
                if let Some(m) = mpd {
                    if let Err(e) = m.pause().await {
                        warn!("mpd pause failed: {e}");
                    }
                } else {
                ...
                player
                    .send_command("set_property", &[json!("pause"), json!(true)])
                    .await;
            }
        }
        PlayerCmd::Resume => {
            if via_mpd {
                if let Some(m) = mpd {
                    if let Err(e) = m.resume().await {
                        warn!("mpd resume failed: {e}");
                    }
                } else {
                    warn!("mpd flagged active but bridge unavailable");
                }
                ...
                player
                    .send_command("set_property", &[json!("pause"), json!(false)])
                    .await;
            }
        }
        PlayerCmd::TogglePause => {
            if via_mpd {
                if let Some(m) = mpd {
                    if let Err(e) = m.toggle_pause().await {
                        warn!("mpd toggle_pause failed: {e}");
                    }
                } else {
                    warn!("mpd flagged active but bridge unavailable");
                }
        ...
        PlayerCmd::Seek { seconds } => {
            if via_mpd {
                if let Some(m) = mpd {
                    if let Err(e) = m.seek_relative(seconds).await {
                        warn!("mpd seek_relative failed: {e}");
                    }
                } else {
                    warn!("mpd flagged active but bridge unavailable");
                }
                ...
                player
                    .send_command("seek", &[json!(seconds), json!("relative")])
                    .await;
            }
        }
        PlayerCmd::SeekAbsolute { seconds } => {
            if via_mpd {
                if let Some(m) = mpd {
                    if let Err(e) = m.seek(seconds).await {
                ...
                player
                    .send_command("seek", &[json!(seconds), json!("absolute")])
                    .await;
            }
        }
        PlayerCmd::SetVolume { level } => {
            if via_mpd {
                if let Some(m) = mpd {
                    if let Err(e) = m.set_volume(level.clamp(0.0, 100.0) as u32).await {
                        warn!("mpd set_volume failed: {e}");
                    }
                } else {
                    warn!("mpd flagged active but bridge unavailable");
                }
                ...
                player
                    .send_command("set_property", &[json!("volume"), json!(level)])
                    .await;
            }
        }
        PlayerCmd::AdjustVolume { delta } => {
            if via_mpd {
                if let Some(m) = mpd {
                    if let Err(e) = m.adjust_volume(delta.clamp(-100.0, 100.0) as i32).await {
                        warn!("mpd adjust_volume failed: {e}");
                    }
                } else {
                    warn!("mpd flagged active but bridge unavailable");
                }
                ...
                player
                    .send_command("add", &[json!("volume"), json!(delta)])
                    .await;
            }
        }
        PlayerCmd::SwitchStream { url } => {
            if via_mpd {
                ...
                tracing::info!(
                    "playback: switch_stream cold-start url={}",
                    &url[..url.len().min(80)]
                );
                player.start_stream_for_switch(&url).await;
            }
        ...
        PlayerCmd::SetSubtitleTrack { id } => {
            player
                .send_command("set_property", &[json!("sid"), json!(id)])
                .await;
        }
        PlayerCmd::AdjustSubtitleDelay { delta } => {
            player
                .send_command("add", &[json!("sub-delay"), json!(delta)])
                .await;
        }
        PlayerCmd::LoadSubtitle { path } => {
            player
                .send_command("sub-add", &[json!(path), json!("select")])
                .await;
        }
        PlayerCmd::SetAudioTrack { id } => {
            player
                .send_command("set_property", &[json!("aid"), json!(id)])
                .await;
        }
        PlayerCmd::AdjustAudioDelay { delta } => {
            player
                .send_command("add", &[json!("audio-delay"), json!(delta)])
                .await;
        }

        // ── Simple mpv-only commands ──────────────────────────────────────
        PlayerCmd::ToggleMute => player.send_command("cycle", &[json!("mute")]).await,
        PlayerCmd::DisableSubtitles => {
            player
                .send_command("set_property", &[json!("sid"), json!("no")])
                .await
        }
        PlayerCmd::CycleSubtitles => player.send_command("cycle", &[json!("sub")]).await,
        PlayerCmd::ResetSubtitleDelay => {
            player
                .send_command("set_property", &[json!("sub-delay"), json!(0)])
                .await
        }
        PlayerCmd::CycleAudioTracks => player.send_command("cycle", &[json!("audio")]).await,
        PlayerCmd::ResetAudioDelay => {
            player
                .send_command("set_property", &[json!("audio-delay"), json!(0)])
                .await
        }
        ...
        PlayerCmd::MpdNext => {
            if let Some(m) = mpd {
                if let Err(e) = m.next().await {
                    warn!("mpd next failed: {e}");
                }
            }
        }
        PlayerCmd::MpdPrev => {
            if let Some(m) = mpd {
                if let Err(e) = m.previous().await {
                    warn!("mpd prev failed: {e}");
                }
            }
        }
        PlayerCmd::MpdShuffle => {
            if let Some(m) = mpd {
                if let Err(e) = m.shuffle().await {
                    warn!("mpd shuffle failed: {e}");
                }
            }
        }
        PlayerCmd::MpdClear => {
            if let Some(m) = mpd {
                if let Err(e) = m.clear().await {
                    warn!("mpd clear failed: {e}");
                }
            }
        }
        PlayerCmd::MpdConsume { enabled } => {
            if let Some(m) = mpd {
                if let Err(e) = m.set_consume(enabled).await {
                    warn!("mpd consume failed: {e}");
                }
            }
        }
        PlayerCmd::ReplayGainMode { mode } => {
            if let Some(m) = mpd {
                if let Err(e) = m.set_replay_gain(&mode).await {
                    warn!("mpd replay-gain failed: {e}");
                }
            }
        }
        PlayerCmd::ToggleMpdOutput { id } => {
            if let Some(m) = mpd {
                if let Err(e) = m.toggle_output(id).await {
                    warn!("mpd toggle-output failed: {e}");
                }
            }
        }
        ...
        PlayerCmd::MpdCrossfade { secs } => {
            if let Some(m) = mpd {
                if let Err(e) = m.set_crossfade(secs).await {
                    warn!("mpd crossfade failed: {e}");
                }
            }
        }
        ...
        PlayerCmd::MpdPlaylistSave { name } => {
            if let Some(m) = mpd {
                if let Err(e) = m.save_playlist(&name).await {
                    warn!("mpd playlist save failed: {e}");
                }
            }
        }
        PlayerCmd::MpdPlaylistLoad { name } => {
            if let Some(m) = mpd {
                if let Err(e) = m.load_playlist(&name).await {
                    warn!("mpd playlist load failed: {e}");
                }
            }
        }
        PlayerCmd::MpdPlaylistAppend { name } => {
            if let Some(m) = mpd {
                if let Err(e) = m.append_playlist(&name).await {
                    warn!("mpd playlist append failed: {e}");
                }
            }
        }
        PlayerCmd::MpdPlaylistDelete { name } => {
            if let Some(m) = mpd {
                if let Err(e) = m.delete_playlist(&name).await {
                    warn!("mpd playlist delete failed: {e}");
                }
            }
        }
        PlayerCmd::MpdPlaylistAddTrack { name, uri } => {
            if let Some(m) = mpd {
                if let Err(e) = m.add_to_playlist(&name, &uri).await {
                    warn!("mpd playlist add-track failed: {e}");
                }
            }
        }
        PlayerCmd::MpdPlaylistCreate { name, uris } => {
            if let Some(m) = mpd {
                if let Err(e) = m.create_playlist(&name, &uris).await {
                    warn!("mpd playlist create failed: {e}");
                }
            }
        }
        PlayerCmd::MpdPlaylistRemoveTrack { name, pos } => {
            if let Some(m) = mpd {
                if let Err(e) = m.remove_from_playlist(&name, pos).await {
                    warn!("mpd playlist remove-track failed: {e}");
                }
            }
        }
        ...
        PlayerCmd::MpdAdd { uri } => {
            if let Some(m) = mpd {
                if let Err(e) = m.add(&uri).await {
                    warn!("mpd add failed: {e}");
                }
            }
        }
        PlayerCmd::MpdRemove { id } => {
            if let Some(m) = mpd {
                if let Err(e) = m.remove_id(id).await {
                    warn!("mpd remove failed: {e}");
                }
            }
        }
        PlayerCmd::MpdPlayId { id } => {
            if let Some(m) = mpd {
                if let Err(e) = m.play_id(id).await {
                    warn!("mpd play_id failed: {e}");
                }
            }
        }
        PlayerCmd::MpdSetVolume { volume } => {
            if let Some(m) = mpd {
                if let Err(e) = m.set_volume(volume).await {
                    warn!("mpd set_volume failed: {e}");
                }
            }
        }
        PlayerCmd::MpdSeek { id, time } => {
            if let Some(m) = mpd {
                if let Err(e) = m.seek_id(id, time).await {
                    warn!("mpd seek failed: {e}");
                }
            }
        }
        ...
        PlayerCmd::MpdUpdate => {
            if let Some(m) = mpd {
                if let Err(e) = m.update_library(None).await {
                    warn!("mpd update failed: {e}");
                }
            }
        }
        PlayerCmd::MpdToggleRepeat => {
            if let Some(m) = mpd {
                if let Err(e) = m.toggle_repeat().await {
                    warn!("mpd toggle_repeat failed: {e}");
                }
            }
        }
        PlayerCmd::MpdToggleSingle => {
            if let Some(m) = mpd {
                if let Err(e) = m.toggle_single().await {
                    warn!("mpd toggle_single failed: {e}");
                }
            }
        }
        PlayerCmd::MpdToggleRandom => {
            if let Some(m) = mpd {
                if let Err(e) = m.toggle_random().await {
                    warn!("mpd toggle_random failed: {e}");
                }
            }
        }
    }

...
impl DitherFilter {
    pub fn new(bit_depth: u32, noise_shaping: NoiseShaping) -> Self {
        ...
        let saw = if noise_shaping == NoiseShaping::Saw {
            ...
            stft_l.add_dsp(Box::new(SawNode::new(default_rate)));
            let mut stft_r = StftProcessor::new(default_rate);
            stft_r.add_dsp(Box::new(SawNode::new(default_rate)));
    ...
    pub fn reset_state(&mut self, rate_hz: u32) {
        ...
        self.entropy_state = [
            EntropyDitherState::new(rate_hz),
            EntropyDitherState::new(rate_hz),
        ];
        ...
        if rate_hz > 0 {
            ...
            match self.noise_shaping {
                NoiseShaping::Saw => {
                    ...
                    l.add_dsp(Box::new(SawNode::new(rate)));
                    let mut r = StftProcessor::new(rate);
                    r.add_dsp(Box::new(SawNode::new(rate)));
    ...
    fn process_sample(&mut self, sample: f32, scale: f32, lsb: f32, ch: usize) -> f32 {
        let tpdf = self.next_tpdf(); // Range [-1, 1], represents ±1 quantization step
        ...
        match &self.noise_shaping {
            ...
            NoiseShaping::Saw => {
                ...
                let tpdf_initial = self.next_tpdf();
                let initial = (sample * scale + tpdf_initial).round() / scale;
                let err = sample - initial;
                let tpdf_final = self.next_tpdf();
                let shaped = if let Some(ref mut saw) = self.saw[ch_idx] {
                    tpdf_final + saw.process(err)
                } else {
                    tpdf_final
                };
                let q = (sample * scale + shaped).round() / scale;
            ...
            NoiseShaping::EntropyDither => {
                // tpdf is now in the [-1, 1] range (raw TPDF), representing ±1 quantization step.
                // Scale by amp_steps (in quantization-step units) for adaptive amplitude.
                let amp_steps = self.entropy_state[ch].update(sample);
                let adaptive_tpdf = tpdf * amp_steps;

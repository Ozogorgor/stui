...
fn compute_a_weights(fft_size: usize, sample_rate: f32) -> Vec<f32> {
    ...
    (0..fft_size)
        .map(|k| {
            10.0_f32
                .powf(a_weight_db(k as f32 * bw) / 20.0)
                .clamp(0.2, 2.0)
        })
        .collect()
}
...
impl SpectralNode for TnsNode {
    fn process_spectrum(&mut self, spectrum: &mut [C], frame_rms: f32, is_transient: bool) {
        ...
        for k in 1..half {
            ...
            // Compute flux using previous smoother value before updating.
            let sm_prev = self.smoothed_mag[k];
            let flux = if sm_prev > 1e-10 {
                (mag - sm_prev).abs() / sm_prev
            } else {
                0.0
            };

            // Update per-bin magnitude smoother (after flux calculation).
...
mod tests {
    ...
    fn transient_bin_suppressed_vs_stable() {
        ...
        assert!(
            rms_stable.is_finite(),
            "stable rms non-finite: {rms_stable}"
        );
        assert!(
            rms_transient.is_finite(),
            "transient rms non-finite: {rms_transient}"
        );

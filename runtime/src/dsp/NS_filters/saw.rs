...
impl TransientDetector {
    ...
    pub fn process(&mut self, rms: f32) -> bool {
        ...
        let db_above = if ratio > 0.0 { 20.0 * ratio.log10() } else { 0.0 };
...
impl SawNode {
    pub fn new(base_alpha: f32, sample_rate: f32) -> Self {
        Self {
            base_alpha,
            min_alpha: 0.4,
            max_alpha: 0.8,
...
impl SpectralNode for SawNode {
    fn process_spectrum(&mut self, spectrum: &mut [C], frame_rms: f32, is_transient: bool) {
        ...
        if self.use_simd {
            unsafe {
                saw_warp_simd(spectrum, alpha, |k: usize| {
                    if k < half { a_weights[k] } else { 1.0 }
                });
            }
            return;
        }
...
/// those frequencies.
fn saw_warp_scalar<F: Fn(usize) -> f32>(
    spectrum: &mut [C],
    half: usize,
    alpha: f32,
    weight_fn: F,
...
impl StereoStftProcessor {
    ...
    pub fn new(sample_rate: f32) -> Self {
        ...
        left.add_dsp(Box::new(SawNode::new(0.6, sample_rate)));
        right.add_dsp(Box::new(SawNode::new(0.6, sample_rate)));
        Self { left, right }
    }

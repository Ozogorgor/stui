...
impl SkipperStore {
    ...
    fn fp_path(&self, series_id: &str, episode_id: &str) -> PathBuf {
        self.series_dir(series_id)
            .join(format!("{}.fp.json", slug(episode_id)))
    }

    fn seg_path(&self, series_id: &str, episode_id: &str) -> PathBuf {
        self.series_dir(series_id)
            .join(format!("{}.seg.json", slug(episode_id)))
    }
    ...
    pub async fn save_fp(
        ...
        let data = StoredFingerprints {
            episode_id: episode_id.to_string(),
            intro_fp,
            credits_fp,
        };
        fs::write(
            self.fp_path(series_id, episode_id),
    ...
    pub async fn load_others(&self, series_id: &str, episode_id: &str) -> Vec<StoredFingerprints> {
        ...
        while let Ok(Some(ent)) = rd.next_entry().await {
            ...
            if !stem.ends_with(".fp") {
                continue;
            }
            let ep_slug = stem.trim_end_matches(".fp");
            if ep_slug == slug(episode_id) {
                continue;
            }
    ...
    pub async fn save_segments(
        ...
        let dir = self.series_dir(series_id);
        fs::create_dir_all(&dir).await?;
        let data = StoredSegments {
            episode_id: episode_id.to_string(),
            intro,
            credits,
        };
        fs::write(
            self.seg_path(series_id, episode_id),
            serde_json::to_string(&data)?,
        )
        .await?;
        Ok(())
    ...
    pub async fn load_segments(&self, series_id: &str, episode_id: &str) -> Option<StoredSegments> {
        let json = fs::read_to_string(self.seg_path(series_id, episode_id))
            .await
            .ok()?;
...
mod tests {
    ...
    async fn test_save_load_fp() {
        ...
        store
            .save_fp("series", "episode1", Some(fp.clone()), None)
            .await
            .unwrap();
    ...
    async fn test_save_load_segments() {
        ...
        store
            .save_segments("series", "ep1", Some(intro), Some(credits))
            .await
            .unwrap();

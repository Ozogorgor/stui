...
pub fn find_common(
    ...
    for i in 0..na {
        for j in 0..nb {
            if hamming(av[i], bv[j]) <= max_dist {
                let prev = if i > 0 && j > 0 { dp[(i - 1) * nb + (j - 1)] } else { 0 };
                let run = prev.saturating_add(1);
                dp[i * nb + j] = run;
                if run > best { best = run; bi = i; bj = j; }
            }
        }
    }
    ...
    if run_secs < min_secs || run_secs > max_secs { return None; }

    let a_start = (bi + 1).saturating_sub(best as usize);
    let b_start = (bj + 1).saturating_sub(best as usize);

    Some((
        Segment { start: a_offset + a_start as f64 / fps_a, end: a_offset + (bi + 1) as f64 / fps_a },
        Segment { start: b_offset + b_start as f64 / fps_b, end: b_offset + (bj + 1) as f64 / fps_b },
    ))
}
...
pub fn detect_segment(
    ...
    for (other_fp, other_offset) in others {
        if let Some((seg, _)) = find_common(
            current, other_fp, current_offset, *other_offset,
            min_secs, max_secs, threshold,
        ) {
            candidates.push(seg);
        }
    }
    ...
    Some(Segment { start: starts[mid], end: ends[mid] })
}

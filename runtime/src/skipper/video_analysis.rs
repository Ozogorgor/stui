...
pub async fn extract_scene_changes(
    url: &str,
    start_secs: f64,
    duration_secs: f64,
) -> Option<SceneInfo> {
    ...
    let task = async {
            ...
            .await
        {
            Ok(o) => o,
            Err(e) => {
                debug!(error = %e, "failed to spawn ffmpeg for scene detection");
                return None;
            }
        };
    ...
    match timeout(deadline, task).await {
        ...
        Ok(None) => {
            debug!(url, "scene detection returned no results");
            None
        }
        Err(_) => {
            warn!(url, "scene detection timed out");
            None
        }
    }
}
...
pub async fn extract_audio_profile(
    url: &str,
    start_secs: f64,
    duration_secs: f64,
) -> Option<AudioProfile> {
    ...
    let task = async {
        let args: Vec<String> = vec![
            ...
            url.to_string(),
            "-af".into(),
            "astats=metadata=1:reset=1,ametadata=print:csv".into(),
            "-f".into(),
            "null".into(),
            "-".into(),
        ];
            ...
            .await
        {
            Ok(o) => o,
            Err(e) => {
                debug!(error = %e, "failed to spawn ffmpeg for audio analysis");
                return None;
            }
        };
    ...
    match timeout(deadline, task).await {
        ...
        Ok(None) => {
            debug!(url, "audio profile extraction returned no results");
            None
        }
        Err(_) => {
            warn!(url, "audio profile extraction timed out");
            None
        }
    }
}
...
pub fn estimate_boundaries(scene_info: &SceneInfo, duration: f64) -> (Option<f64>, Option<f64>) {
    ...
    let credits_start = scene_info.cuts.iter().rev().find(|&&t| t < duration - 60.0);

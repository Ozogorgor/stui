...
impl PlayerBridge {
    pub fn new(
        ...
        PlayerBridge {
            mpv, torrents, mpd, engine, config, storage, watch_history, ipc_tx,
            data_dir, playback_cfg, dsp,
    ...
    async fn play_via_torrent(
        ...
        if let Err(e) = self.mpv.play_idle(
            title, sub, &self.data_dir,
            &extra_flags,
        ...
        let _ = self.mpv.send_command(&serde_json::json!([
            "script-message", "stui-status", "Fetching torrent metadata…"
        ])).await;
        let _ = self.mpv.send_command(&serde_json::json!([
            "script-message", "stui-busy", "on"
        ])).await;
        ...
        let stream_url = match self.torrents.start_stream(uri).await {
            ...
            Err(e) => {
                ...
                let _ = self.mpv.send_command(&serde_json::json!([
                    "script-message", "stui-busy", "off"
                ])).await;
                let _ = self.mpv.send_command(&serde_json::json!([
                    "script-message", "stui-status", format!("Torrent failed: {e}")
                ])).await;

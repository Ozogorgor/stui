    ...
    pub fn badge(&self) -> String {
        ...
        let hdr_label = match s.hdr {
            ...
            HdrFormat::None        => {
                ...
                if n.contains("DOLBY VISION") || n.contains(" DV ") { Some("DV") }
                else if n.contains("HDR10+") { Some("HDR10+") }
                else if n.contains("HDR") { Some("HDR") }
                else { None }
            }
        };
...
fn extract_codec(name: &str) -> Option<String> {
    ...
    if n.contains("AV1")                         { return Some("AV1".into()); }
    if n.contains("HEVC") || n.contains("H265") || n.contains("X265") { return Some("HEVC".into()); }
    if n.contains("H264") || n.contains("X264") || n.contains("AVC")  { return Some("H264".into()); }
    None
}

fn extract_source(name: &str) -> Option<String> {
    let n = name.to_uppercase();
    if n.contains("BLURAY") || n.contains("BLU-RAY") || n.contains("BDREMUX") { return Some("BluRay".into()); }
    if n.contains("WEBDL") || n.contains("WEB-DL")   { return Some("WEB-DL".into()); }
    if n.contains("WEBRIP") || n.contains("WEB-RIP") { return Some("WEBRip".into()); }
    if n.contains("HDTV")                            { return Some("HDTV".into()); }
    if n.contains("DVDRIP") || n.contains("DVD-RIP") { return Some("DVDRip".into()); }
    if n.contains("CAM") || n.contains("HDCAM")      { return Some("CAM".into()); }
    None
}

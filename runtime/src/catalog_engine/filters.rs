...
impl Filter {
    pub fn genre(s: impl Into<String>) -> Self { Filter::Genre(s.into()) }
    pub fn year_range(from: u32, to: u32) -> Self { Filter::YearRange(from, to) }
    pub fn media_type(t: MediaType) -> Self { Filter::MediaType(t) }
    pub fn min_rating(r: f64) -> Self { Filter::MinRating(r) }
    pub fn provider(p: impl Into<String>) -> Self { Filter::Provider(p.into()) }
    pub fn title_contains(s: impl Into<String>) -> Self { Filter::TitleContains(s.into()) }

    pub fn matches(&self, entry: &CatalogEntry) -> bool {
        match self {
            Filter::Genre(g) => entry.genre.as_deref()
                .map(|genre| genre.to_lowercase().contains(&g.to_lowercase()))
                .unwrap_or(true), // keep if unknown

            Filter::YearRange(from, to) => entry.year.as_deref()
            ...
            Filter::MinRating(min) => entry.rating.as_deref()
                .and_then(|r| r.parse::<f64>().ok())
                .map(|r| r >= *min)
                .unwrap_or(true),

            Filter::Provider(p) => entry.provider
                .split(',')
                .any(|prov| prov.trim() == p.as_str()),

            Filter::TitleContains(s) => entry.title
                .to_lowercase()
...
impl FilterSet {
    pub fn new() -> Self { FilterSet::default() }

    pub fn add(&mut self, f: Filter) { self.filters.push(f); }

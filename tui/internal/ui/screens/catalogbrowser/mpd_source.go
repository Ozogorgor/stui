...
func (s *MpdDataSource) Items(kind ipc.EntryKind) []Entry { return s.items[kind] }
...
// applyMpdResult overwrites items with what MPD returned.
func (s *MpdDataSource) applyMpdResult(r *ipc.MpdSearchResult) {
	s.items = map[ipc.EntryKind][]Entry{
		ipc.KindArtist: MapMpdArtists(r.Artists),
		ipc.KindAlbum:  MapMpdAlbums(r.Albums),
		ipc.KindTrack:  MapMpdSongs(r.Tracks),
	}
}
